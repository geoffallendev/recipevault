...
perl -pe "s|(\"Resource\": \".*?:::)[^\"/]*|\$1$bucket|" -i deploy/s3/public_s3.json
...
aws s3api put-bucket-policy --no-verify-ssl --endpoint-url "https://$s3route" --bucket $bucket --policy "$(cat deploy/s3/public_s3.json)"
...
# frontend
perl -pe "s|(VUE_APP_IMAGE_SERVER_URL:).*$|\$1 http://$(oc get route/s3 -n openshift-storage -o jsonpath='{.spec.host}')/$bucket/|" -i deploy/ui-recipevault/recipevault-ui-config.yaml
